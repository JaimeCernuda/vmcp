...
class ExtensionManager:
    ...
    def __init__(self, vmcp_home: str = "~/.vmcp"):
        ...
        self._enabled_extensions: Optional[Dict[str, Any]] = None
            ...
            if repo_dir.is_dir():
                ...
                if repo_manifest.exists():
                    try:
                        ...
                        logger.warning(f"Failed to load repository {repo_dir.name}: {e}")
        
        return repositories
    
    def list_available_extensions(self, repository_id: str = "builtin") -> List[Dict[str, Any]]:
        """
            ...
            if ext_dir.is_dir():
                ...
                if manifest_path.exists():
                    try:
                        ...
                        manifest["enabled"] = self.is_extension_enabled(manifest.get("name", ""))
                        
                        installed.append(manifest)
                    except Exception as e:
                        logger.warning(f"Failed to load extension manifest {ext_dir.name}: {e}")
        
        return installed
    
    def install_extension(self, extension_id: str, repository_id: str = "builtin") -> bool:
        """
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if not manifest_path.exists():
                    raise ExtensionError(f"Invalid extension package: missing manifest.json")
                ...
                if manifest.get("name") != extension_id:
                    raise ExtensionError(f"Extension ID mismatch: expected {extension_id}, got {manifest.get('name')}")
    ...
    def enable_extension(self, extension_id: str, config: Optional[Dict[str, Any]] = None) -> bool:
        """
        Enable an extension.
        ...
        if not manifest_path.exists():
            raise ExtensionError(f"Invalid extension: missing manifest.json")
    ...
    def update_extension(self, extension_id: str, repository_id: str = "builtin") -> bool:
        """
        ...
        enabled_config = self._load_enabled_extensions()
        ext_config = enabled_config.get("enabled", {}).get(extension_id, {}).get("config", {})
