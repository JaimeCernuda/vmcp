...
class TestCLIIntegration:
    ...
    def test_extension_list(self):
        ...
        result = run(["uv", "run", "vmcp", "extension", "list"], capture_output=True, text=True)
        assert result.returncode == 0
    ...
    def test_completion_generation(self):
        ...
        result = run(["uv", "run", "vmcp", "completion", "bash"], capture_output=True, text=True)
        assert result.returncode == 0
        assert "complete" in result.stdout
        
        # Test zsh completion
        result = run(["uv", "run", "vmcp", "completion", "zsh"], capture_output=True, text=True)
        assert result.returncode == 0
    ...
    def test_repo_commands(self):
        ...
        # Test repo stats
        result = run(["uv", "run", "vmcp", "repo", "stats"], capture_output=True, text=True)
        assert result.returncode == 0
        
        # Test repo search
        result = run(["uv", "run", "vmcp", "repo", "search", "pandas"], capture_output=True, text=True)
        assert result.returncode == 0
    ...
    def test_extension_install_uninstall_cycle(self):
        ...
        run(["uv", "run", "vmcp", "extension", "uninstall", extension_id, "--force"], 
            capture_output=True)
    ...
    def test_health_and_metrics_commands(self):
        ...
        result = run(["uv", "run", "vmcp", "health", "check"], capture_output=True, text=True)
        assert result.returncode in [0, 1]  # Allow failure when no servers
        
        # Test metrics show
        result = run(["uv", "run", "vmcp", "metrics", "show"], capture_output=True, text=True)
        assert result.returncode in [0, 1]  # Allow failure when no servers

    def test_error_handling(self):
        """Test CLI error handling."""
        # Test invalid command
        result = run(["uv", "run", "vmcp", "invalid-command"], capture_output=True, text=True)
        assert result.returncode != 0
